...
exports.register = function (plugin, options, next) {
  ...
  internals.getHoodiePath = function (request, reply) {
	...
	internals.getCordovaPath = function (request) {
        var userAgent = String(request.headers["user-agent"]).toLowerCase();
        var isAndroid = userAgent.indexOf("android") !== -1;
        var isIOs = userAgent.indexOf("iphone") !== -1;
        if (isAndroid) {
            return path.join(__dirname, '../../../../../../cordovaPart/platforms/android/assets/www');
        } else if (isIOs) {
            return path.join(__dirname, '../../../../../../cordovaPart/platforms/ios/www');
        }
        return "";
    };

    internals.getCordova = function (request, reply) {
        var pathToCordova = path.join(internals.getCordovaPath(request), "cordova.js");
        fs.readFile(pathToCordova, function (err, data) {
            if (err) {
                console.log(err);
                return reply({
                    'error': 'not found'
    ...
    internals.getCordovaPlugins = function (request, reply) {
        var pathToCordova = path.join(internals.getCordovaPath(request), "cordova_plugins.js");
        fs.readFile(pathToCordova, function (err, data) {
            if (err) {
                console.log(err);
                return reply({
    ...
    internals.getCordovaPlugin = function (request, reply) {
        var pluginPath = request.params.path;
        var pathToCordova = path.join(internals.getCordovaPath(request), "plugins", pluginPath);
        fs.readFile(pathToCordova, function (err, data) {
            if (err) {
                console.log(err);
                return reply({
    ...
    },
    {
      ...
      handler: function (request, reply) {
    ...
    },
        {
            method: 'GET',
            path: '/cordova.js',
            handler: internals.getCordova
        },
        {
            method: 'GET',
            path: '/cordova_plugins.js',
            handler: internals.getCordovaPlugins
        },
        {
            method: 'GET',
            path: '/plugins/{path*}',
            handler: internals.getCordovaPlugin
        }
  ]);
