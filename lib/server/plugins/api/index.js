...
exports.register = function (plugin, options, next) {
  ...
  internals.getHoodiePath = function (request, reply) {
    ...
    internals.getCordova = function (request, reply) {
		var tmp = internals.getCordovaPath(request);
        if (!tmp) {
			return reply('/* Empty file haha */')
                .type('application/javascript')
                .header(
                    'Pragma', 'no-cache'
                );
		}
		var pathToCordova = path.join(tmp, "cordova.js");
    ...
    internals.getCordovaPlugins = function (request, reply) {
		...
		var pathToCordova = path.join(tmp, "cordova_plugins.js");
        fs.readFile(pathToCordova, function (err, data) {
            if (err) {
                console.log(err);
                return reply({
    ...
    internals.getCordovaPlugin = function (request, reply) {
        ...
        var pathToCordova = path.join(tmp, "plugins", pluginPath);
        fs.readFile(pathToCordova, function (err, data) {
            if (err) {
                console.log(err);
                return reply({
